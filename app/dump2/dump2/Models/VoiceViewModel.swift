...
class VoiceViewModel: NSObject, ObservableObject, AVAudioPlayerDelegate {
  ...
  func startRecording() {
    ...
    curFileName = path.appendingPathComponent(
      "rec-\(Date().toString(dateFormat: "dd-MM-YY_HH-mm-ss")).m4a")
    ...
    do {
      ...
      timerCount = Timer.scheduledTimer(
  ...
  func fetchAllRecording() {
    ...
    for i in directoryContents {
      recordingsList.append(
        Recording(fileURL: i, createdAt: getFileDate(for: i), isPlaying: false)
      )
    }
  ...
  func blinkColor() {
    ...
    blinkingCount = Timer.scheduledTimer(
